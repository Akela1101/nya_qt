...
static const char salt256[] = "~el~psy~congroo!~~el~psy~congroo!~"; // 34

static EVP_CIPHER_CTX cAES;
static const uchar keyAES[32] = "~el~psy~congroo~el~psy~congroo~"; // 31 + 0
static const uchar ivAES[32]  = "~nyan~nyan~nyan~nyan~nyan~nyan~"; // 31 + 0
 ...
 */
QByteArray Crypto::GetHash256(const QString& s)
{
	QByteArray ret;
	ret.resize(SHA256_DIGEST_LENGTH);

	// Пароль = пароль + соль
	QByteArray sArray = s.toUtf8() + salt256;
